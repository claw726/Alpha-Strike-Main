...
async function fetchAndDisplayMetric(metric, tableId, timeRange = 'month') {
    ...
    try {
        ...
        if (!apiData || typeof apiData !== 'object' || !apiData[metric]) { // Added typeof apiData check
            console.error(`No data returned or invalid structure for metric: ${metric} in API response:`, apiData);
            const preferredLang = localStorage.getItem('preferredLanguage') || 'en';
            const noDataText = translations["error.noData"]?.[preferredLang] || `No data available for metric: ${metric}`;
            if (dataContainer) {
...
function displayMetricDataInTable(metricData, table, tableBody, metric, dataContainer, timeRange) {
    ...
    if (metricData && Array.isArray(metricData) && metricData.length > 0) {
        ...
        if (dataContainer) {
            dataContainer.innerHTML = `<p data-translate="error.noData">${translations["error.noData"]?.[localStorage.getItem('preferredLanguage') || 'en'] || "No data available for this metric."}</p>`;
            table.style.display = "none"; 
        }
    }
    addTableClickListeners(table.id);
    if (typeof setLanguage === 'function') { // Ensure dynamic content is re-translated
