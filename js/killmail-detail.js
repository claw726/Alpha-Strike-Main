...
function createCombatantCard(data, type) {
  ...
  image.alt = getTranslationLocal(`altText.${type}Image`, `${type} ${data.name}`, {
    name: data.name,
  });
  ...
  name.title = getTranslationLocal('tooltip.searchFor', 'Search for {itemName}', {
    itemName: data.name,
  });
...
function createEngagementDetails(data) {
  ...
  locationSpan.title = getTranslationLocal('tooltip.searchFor', 'Search for {itemName}', {
...
function createFittingSection() {
  ...
  const categories = [
    ...
    {
      title: getTranslationLocal('killmail.fitting.ammoCharges', 'Ammunition / Charges'),
      icon: 'cubes',
      items: [
...
function renderKillmailDetailsHTML(data) {
  ...
  killerSpan.title = getTranslationLocal('tooltip.searchFor', 'Search for {itemName}', {
  ...
  victimSpan.title = getTranslationLocal('tooltip.searchFor', 'Search for {itemName}', {
    itemName: victimName,
  });
  ...
  systemSpan.title = getTranslationLocal('tooltip.searchFor', 'Search for {itemName}', {
...
async function loadAndRenderKillmailPageContent() {
  ...
  if (mail_id) {
    try {
      ...
      if (!killmailDataStore || killmailDataStore.id !== Number.parseInt(mail_id)) {
