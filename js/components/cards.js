...
function createPlayerCardHeader(item, stats) {
    ...
    kdStat.innerHTML = `
        <div class="header-stat-value ${stats.killDeathRatio > 1 ? 'killer' : 'victim'}">${stats.killDeathRatio}</div>
...
function calculateSystemStats(item) {
    const avgIncidentsPerDay = item.incident_count && item.days_active
        ? (item.incident_count / item.days_active).toFixed(1)
        : "< 1";
    const threatLevel = item.incident_count > 100
        ? "HIGH"
        : item.incident_count > 50
            ? "MEDIUM"
            : "LOW";
    const threatClass = threatLevel === "HIGH"
        ? "threat-high"
        : threatLevel === "MEDIUM"
            ? "threat-medium"
            : "threat-low";
